...
sayHelloButton.addEventListener('mouseout', changeButtonColorBack)
...
const ohMy = () => {
    axios.get('http://localhost:3000/.jake')
    .then(res =>{
...
// PROBLEM 6 
/*
    Now lets see if you can send a request param! inside repeatMyParam function below  make get request to {SOMEPARAM}', but with a string instead of {SOMEPARAM}.  
...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/Boring`)
    .then(res => {
...
*/
// localhost:3000/user/name/aga
// CODE HERE
const getRequest = ()=>{
    axios.get('http://localhost:3000/query-test?query=say-hello')
    .then(res => {
        console.log(res)
    })
...
const createFood = () =>{
    ...
    let foodInput = document.querySelector('.foodRequest')
    const body = {
